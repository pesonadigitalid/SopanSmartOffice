...
$cek2 = $db->get_row("SELECT * FROM tb_assign WHERE IDAssign='$idAssign'");
if ($cek2) {
    $query = $db->query("UPDATE tb_assign SET Status='1', DateApproved=NOW(), DateModified=NOW(), ModifiedBy='" . $_SESSION["uid"] . "', ApproveMethod='3', ApprovedBy='" . $_SESSION["uid"] . "' WHERE IDAssign='$idAssign'");
    if ($query) {
        //UPDATE LOKASI
