...
$return = array();
$query = $db->get_results("SELECT *,DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_surat_masuk $cond AND Status>'0' ORDER BY IDSuratMasuk DESC");
if ($query) {
    $i = 0;
    foreach ($query as $data) {
        ...
        array_push($return, array("IDSuratMasuk" => $data->IDSuratMasuk, "No" => $i, "NoSurat" => $data->NoSurat, "Proyek" => $proyek->NamaProyek, "KodeProyek" => $proyek->KodeProyek, "TahunProyek" => $proyek->Tahun, "Tanggal" => $data->TanggalID, "Prihal" => $data->Prihal, "FileSurat" => $data->FileSurat, "Deskripsi" => $data->Deskripsi, "Jenis" => $data->Jenis, "Departement" => $departement));
    }
}
