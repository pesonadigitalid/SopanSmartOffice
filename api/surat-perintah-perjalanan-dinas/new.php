...
$dataLast = $db->get_row("SELECT * FROM tb_surat_perintah_perjalanan_dinas ORDER BY IDSPPD DESC");
if($dataLast){
    $last = substr($dataLast->NoSPPD,-5);
    $last++;
    if($last<10000 and $last>=1000)
        ...
        $last = "0000".$last;
    $no_sppd = "LD/SPPD/".date("Y")."/".$last;  
} else {
    $no_sppd = "LD/SPPD/".date("Y")."/00001"; 
}

$query = $db->query("INSERT INTO tb_surat_perintah_perjalanan_dinas SET NoSPPD='$no_sppd', Tanggal='$tanggal', IDKaryawan='$karyawan', NamaPerusahaan='$nama_perusahaan', Alamat='$alamat', Telp='$no_telp', RencanaTugas='$rencana_tugas', TanggalMulai='$tgl_mulai', TanggalAkhir='$tgl_akhir', JamMulai='$jam_mulai', JamAkhir='$jam_akhir', Catatan='$catatan', Status='$statusspk', CreatedBy='$iduser'");
if($query){
    echo "1";
} else {
    echo "0";
