...
$SPPD = array();
$query = $db->get_results("SELECT *, DATE_FORMAT(TanggalMulai, '%d/%m/%Y') AS TanggalMulaiID, DATE_FORMAT(TanggalAkhir, '%d/%m/%Y') AS TanggalAkhirID FROM tb_surat_perintah_perjalanan_dinas $cond ORDER BY IDSPPD ASC");
if($query){
    $i=0;
    foreach($query as $data){
        ...
        array_push($SPPD,array("IDSPPD"=>$data->IDSPPD,"NoSPPD"=>$data->NoSPPD,"TanggalMulai"=>$data->TanggalMulaiID,"TanggalAkhir"=>$data->TanggalAkhirID,"Karyawan"=>$karyawan->Nama,"Bagian"=>$karyawan->Jabatan,"No"=>$i,"NamaPerusahaan"=>$data->NamaPerusahaan,"Status"=>$status));
    }
}
...
$return = array("SPPD"=>$SPPD, "Karyawan"=>$Karyawan);
