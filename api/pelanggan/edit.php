...
$nama_npwp = antiSQLInjection($_POST['nama_npwp']);
$no_npwp = antiSQLInjection($_POST['no_npwp']);
$alamat_npwp = antiSQLInjection($_POST['alamat_npwp']);

if($status=="") $status="0";
if(strlen($kode_pelanggan)>10){
    echo "2";
} else {
    $query = $db->query("UPDATE tb_pelanggan SET KodePelanggan='$kode_pelanggan',NamaPelanggan='$nama', Alamat='$alamat', Kota='$kota', Provinsi='$provinsi', KodePos='$kode_pos', NoTelp='$no_telp', NoFax='$no_fax', Email='$email', Website='$website', Kategori='$kategori', Status='$status', Jenis='$jenis', NamaKP1='$namakp1', JabatanKP1='$jabatankp1', EmailKP1='$emailkp1', HPKP1='$hpkp1', NamaKP2='$namakp2', JabatanKP2='$jabatankp2', EmailKP2='$emailkp2', HPKP2='$hpkp2', NoNPWP='$no_npwp', NamaNPWP='$nama_npwp', AlamatNPWP='$alamat_npwp' WHERE IDPelanggan='$id'");
    if($query){
        echo "1";
    } else {
        echo "0";
