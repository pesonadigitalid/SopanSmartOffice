...
switch ($act) {
    ...
    case "DataList":
        ...
        $query = $db->get_results("SELECT *,DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_kas_masuk $cond ORDER BY NoBukti ASC");
        if($query){
            $i=0;
            foreach($query as $data){
                $i++;
                $created = $db->get_var("SELECT Nama FROM tb_karyawan WHERE IDKaryawan='".$data->CreatedBy."'");
                array_push($return,array("IDKasMasuk"=>$data->IDKasMasuk,"NoBukti"=>$data->NoBukti,"No"=>$i,"Tanggal"=>$data->TanggalID,"Jumlah"=>$data->Jumlah,"Keterangan"=>$data->Keterangan,"DariRekening"=>$data->DariRekening,"KeRekening"=>$data->KeRekening,"ContactPerson"=>$data->ContactPerson,"CreatedBy"=>$created));
            }
        }
    ...
    case "LoadAllRequirement":
        ...
        $query = $db->get_results("SELECT * FROM tb_master_rekening WHERE IDRekening='153' AND Tipe='D'");
        if($query){
            foreach($query as $data){
    ...
    case "InsertNew":
        ...
        $dataLast = $db->get_row("SELECT * FROM tb_kas_masuk WHERE DATE_FORMAT(Tanggal,'%Y-%m')='".$tanggalCond."' ORDER BY NoBukti DESC");
        if($dataLast){
            ...
            if($last<100 and $last>=10)
                ...
                $last = "00".$last;
            $nobukti = "KM".$tanggalCond2.$last;
        } else {
            $nobukti = "KM".$tanggalCond2."001";
        }

        $query = $db->query("INSERT INTO tb_kas_masuk SET NoBukti='$nobukti', Tanggal='$tanggal', Jumlah='$jumlah_pembayaran', Keterangan='$keterangan', DariRekening='$darirekening', KeRekening='$kerekening', CreatedBy='".$_SESSION["uid"]."', ContactPerson='$cp'");
        if($query){
            echo "1";
    ...
    case "Delete":
        ...
        $data = $db->get_row("SELECT * FROM tb_kas_masuk WHERE IDKasMasuk='$idr'");

        $db->query("DELETE FROM tb_kas_masuk WHERE IDKasMasuk='$idr'");
    ...
    case "loadData":
        ...
        $query = $db->get_results("SELECT * FROM tb_master_rekening WHERE IDRekening='153'");
        if($query){
            foreach($query as $data){
                array_push($dariRekening, array("IDRekening"=>$data->IDRekening,"KodeRekening"=>$data->KodeRekening,"NamaRekening"=>$data->NamaRekening));
            }
        ...
        $query = $db->get_results("SELECT * FROM tb_master_rekening WHERE IDRekening='112'");
        if($query){
            foreach($query as $data){
                array_push($keRekening, array("IDRekening"=>$data->IDRekening,"KodeRekening"=>$data->KodeRekening,"NamaRekening"=>$data->NamaRekening));
            }
        ...
        $data = $db->get_row("SELECT *,DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_kas_masuk WHERE NoBukti='$id'");
        if($data){
            $detail = array("IDKasMasuk"=>$data->IDKasMasuk,"NoBukti"=>$data->NoBukti,"No"=>$i,"Tanggal"=>$data->TanggalID,"Jumlah"=>$data->Jumlah,"Keterangan"=>$data->Keterangan,"DariRekening"=>$data->DariRekening,"KeRekening"=>$data->KeRekening,"ContactPerson"=>$data->ContactPerson);
        }
    ...
    case "Edit":
        ...
        $query = $db->query("UPDATE tb_kas_masuk SET Tanggal='$tanggal', Jumlah='$jumlah_pembayaran', Keterangan='$keterangan', DariRekening='$darirekening', KeRekening='$kerekening', CreatedBy='".$_SESSION["uid"]."', ContactPerson='$cp' WHERE NoBukti='$nobukti'");
        if($query){
            echo "1";
