...
$query = $db->get_results("SELECT *, DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_jurnal WHERE NoRef='$id' AND Tipe='4' ORDER BY NoBukti ASC, Tanggal ASC");
if($query){
    $i=0;
    foreach($query as $data){
        ...
        array_push($dataPembayaran,array("NoPembayaran"=>$data->NoBukti,"Tanggal"=>$data->TanggalID,"No"=>$i,"Jumlah"=>$data->Debet,"Bank"=>$detail,"UserName"=>$by));
    }
}

$query = $db->get_row("SELECT * FROM tb_penjualan_invoice WHERE IDInvoice='$id'");
