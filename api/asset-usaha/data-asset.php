...
$query = $db->get_results("SELECT *, DATE_FORMAT(JatuhTempoUsaha,'%d/%m/%Y') AS JatuhTempoUsahaID FROM tb_asset WHERE Jenis='Ijin-Usaha' $cond ORDER BY IDAsset ASC");
if ($query) {
    $i = 1;
    foreach ($query as $data) {
        ...
        if ($allow) {
            ...
            array_push($assetArray, array("IDAsset" => $data->IDAsset, "No" => $i, "KodeAsset" => $data->KodeAsset, "Category" => $category, "Nama" => $data->Nama, "IDKaryawan" => $data->IDKaryawan, "IDProyek" => $data->IDProyek, "Karyawan" => $karyawan, "Unit" => $data->Unit, "Status" => $status, "TahunBeli" => $tahun, "TanggalAssign" => $tanggal_assign, "Proyek" => $proyek, "JatuhTempoUsaha" => $data->JatuhTempoUsahaID, "NoIjinUsaha" => $data->NoIjinUsaha));
            $i++;
        }
    }
}
...
$query = $db->get_results("SELECT * FROM tb_asset_category WHERE Jenis='Ijin-Usaha' ORDER BY IDAssetCategory ASC");
if ($query) {
    $i = 1;
    foreach ($query as $data) {
