...
<html>
    ...
    <body>
        ...
        <div class="laporanTitle">
            <h1 class="underline">** LAPORAN DATA RETURN SUPPLIER **</h1><?php echo $subtitle; ?>
        </div>
        <table class="tabelList2" cellpadding="0" cellspacing="0">
            <thead>
                <tr>
                    <th width="100">No. Return Supplier</th>
                    <th>Supplier</th>
            ...
            <tbody>
                <?php
                $query = newQuery("get_results","SELECT *, DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_return_po $cond ORDER BY IDReturn ASC");
                if($query){
                    $i=1;
                    $totalItem = 0;
                    foreach($query as $data){
                        $nsupplier = $db->get_var("SELECT NamaPerusahaan FROM tb_supplier WHERE IDSupplier='".$data->IDSupplier."'");
                        $gtotalItem = $db->get_var("SELECT SUM(Qty) FROM tb_return_po_detail WHERE NoReturn='".$data->NoReturn."'");
                        ?>
                        <tr>
