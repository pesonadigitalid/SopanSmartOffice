...
<html>
    ...
    <body>
        ...
        <div class="laporanTitle">
            <h1 class="underline">** LAPORAN DATA PENERIMAAN BARANG **</h1><?php echo $subtitle; ?>
        </div>
        <table class="tabelList2" cellpadding="0" cellspacing="0">
            <thead>
                <tr>
                    <th width="100">No.Penerimaan</th>
            ...
            <tbody>
                <?php
                $query = newQuery("get_results","SELECT *,DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_penerimaan_stok $cond ORDER BY NoPenerimaanBarang ASC");
                if($query){
                    $i=1;
                    $totalItem = 0;
                    foreach($query as $data){
                        ?>
                        <tr>
                            <td><strong><?php echo $data->NoPenerimaanBarang;?></strong></td>
                            <td><?php echo $data->NoPO;?></td>
                            <td style="text-align: center;"><?php echo $data->TotalQty;?></td>
                        </tr>
                        <?php
                        $i++;
                        $totalItem += $data->TotalQty;
                    }
                } else {
