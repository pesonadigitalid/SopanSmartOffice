...
<body>
    ...
    <div class="laporanTitle">
        <h1 class="underline">** REKAP PO PAJAK <?php echo $subjenis; ?>**</h1><?php echo $subtitle; ?>
    </div>
    <table class="tabelList2" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th width="100">No Faktur Pajak</th>
                <th width="100">Pajak Terbayar</th>
                <th width="100">Sisa Pajak</th>
            </tr>
        </thead>
        <tbody>
            <?php
            $query = newQuery("get_results", "SELECT *,DATE_FORMAT(Tanggal, '%d/%m/%Y') AS TanggalID FROM tb_po $cond AND IsPajak='1' AND DeletedDate IS NULL ORDER BY NoPO ASC");
            if ($query) {
                $i = 1;
                $totalItem = 0;
                $totalNilai = 0;
            ...
            ?>
                    <tr>
                        ...
                        <td style="text-align: left;">
                            <?php
                            $totalPajakTerbayar = 0;
                            $qPajak = $db->get_results("SELECT * FROM tb_po_faktur_pajak WHERE IDPO='" . $data->IDPO . "' ORDER BY Tanggal ASC");
                            if ($qPajak) {
                                foreach ($qPajak as $dPajak) {
                                    echo $dPajak->NoFaktur . "<br/>";
                                    $totalPajakTerbayar += $dPajak->Nilai;
                                }
                            }
                            ?>
                        </td>
                        <td style="text-align: right;"><?php echo number_format($totalPajakTerbayar, 2); ?></td>
                        <td style="text-align: right;"><?php echo number_format(($data->PPN - $totalPajakTerbayar), 2); ?></td>
                    </tr>
            <?php
                    $i++;
                    $totalItem += $totalItem;
                    ...
                    $sisa += $data->Sisa;
                    $ttotalPajakTerbayar += $totalPajakTerbayar;
                    $tsisa += ($data->PPN - $totalPajakTerbayar);
                }
            } else {
                echo "<td colspan='13'>Tidak ada data yang dapat ditampilkan...</td>";
            }
            ?>
            ...
            <tr>
                ...
                <td></td>
                <td style="text-align: right;"><?php echo number_format($ttotalPajakTerbayar, 2); ?></td>
                <td style="text-align: right;"><?php echo number_format($tsisa, 2); ?></td>
            </tr>
        </tbody>
    </table>
