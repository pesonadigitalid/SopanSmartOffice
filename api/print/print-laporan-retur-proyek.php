...
<html>
    ...
    <body>
        ...
        <?php
        $qProyek = newQuery("get_results","SELECT * FROM tb_proyek $cond2");
        if($qProyek){
            foreach($qProyek as $dProyek){
                $i = 0;
                $numKar = newQuery("get_var","SELECT COUNT(*) FROM tb_audit WHERE $cond AND IDProyek='".$dProyek->IDProyek."'");
                if($numKar>0 || $id_proyek==$dProyek->IDProyek){
                    $totalNilai = 0;
                    $totalQty = 0;
                    ?>
                    <table class="tabelList6" cellpadding="0" cellspacing="0">
                        <thead>
                            <tr>
                                <th colspan="5" style="text-align: left">PROYEK : <?php echo $dProyek->KodeProyek; ?>/<?php echo $dProyek->Tahun; ?> - <?php echo $dProyek->NamaProyek; ?></th>
                            </tr>
                            <tr>
                                <th width="5">No.</th>
                        ...
                        <tbody>
                            <?php
                            $query = newQuery("get_results","SELECT *, SUM(SPGudang) AS QtyRetur FROM tb_audit_detail WHERE NoAudit IN (SELECT NoAudit FROM tb_audit WHERE $cond AND IDProyek='".$dProyek->IDProyek."') GROUP BY IDBarang ORDER BY NamaBarang");
                            if($query){
                                foreach($query as $data){
                                    if($data->QtyRetur>0){
                                        ...
                                        $totalQty += $data->SPGudang;
                                        $totalNilai += $data->SubTotal;
                                        ?>
                                        <tr>
                                            <td><?php echo $i; ?></td>
                                            <td style="text-align: center"><?php echo $dBarang->KodeBarang; ?></td>
                                            <td><?php echo $dBarang->Nama; ?></td>
                                            <td style="text-align: right"><?php echo number_format($data->QtyRetur,2); ?></td>
                        ...
                        <tfooter>
                            <tr>
                                <th colspan="3" style="text-align: right;">JUMLAH RETUR: </th>
