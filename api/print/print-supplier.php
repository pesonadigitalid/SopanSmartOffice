...
<body>
    ...
    <div class="laporanTitle">
        <h1 class="underline">** DATA SUPPLIER **</h1>
    </div>
    <div class="data-container">
        <?php
        $query = $db->get_row("SELECT a.*, b.NamaDepartement FROM tb_supplier a, tb_departement b  WHERE a.Kategori=b.IDDepartement AND a.IDSupplier='$id' ORDER BY a.IDSupplier ASC");

        if ($query->NamaKP1 == "") $namakp1 = "-";
        else $namakp1 = $query->NamaKP1;
        if ($query->JabatanKP1 == "") $jabatankp1 = "-";
        else $jabatankp1 = $query->JabatanKP1;
        if ($query->EmailKP1 == "") $emailkp1 = "-";
        else $emailkp1 = $query->EmailKP1;
        if ($query->HPKP1 == "") $hpkp1 = "-";
        else $hpkp1 = $query->HPKP1;
        if ($query->NamaKP2 == "") $namakp2 = "-";
        else $namakp2 = $query->NamaKP2;
        if ($query->JabatanKP2 == "") $jabatankp2 = "-";
        else $jabatankp2 = $query->JabatanKP2;
        if ($query->EmailKP2 == "") $emailkp2 = "-";
        else $emailkp2 = $query->EmailKP2;
        if ($query->HPKP2 == "") $hpkp2 = "-";
        else $hpkp2 = $query->HPKP2;
        ?>
        <fieldset>
            <label><strong>* Kode Pelanggan</strong></label>
            <span style="margin-right: 5px;">:</span><span><?php echo $query->KodeSupplier; ?></span>
        </fieldset>
        <fieldset>
        ...
        <!-- <fieldset>
            <label><strong>* Jenis Produk</strong></label>
            <span style="margin-right: 5px;">:</span><span><?php echo $query->Jenis; ?></span>
        </fieldset> -->
        <fieldset>
            <label><strong>1. Nama Perusahaan</strong></label>
            <span style="margin-right: 5px;">:</span><span><?php echo $query->NamaPerusahaan; ?></span>
        </fieldset>
        <fieldset>
        ...
        <fieldset>
            <label><strong>8. Deskripsi Produk</strong></label>
            <span style="margin-right: 5px;">:</span><span><?php echo $query->Deskripsi; ?></span>
        </fieldset>
