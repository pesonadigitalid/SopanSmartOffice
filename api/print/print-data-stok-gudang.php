...
<body>
    ...
    <div class="laporanTitle">
        <h1 class="underline">** LAPORAN STOK BARANG **</h1><?php echo $periode; ?>
    </div>
    <table class="tabelList2" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php
            $query = newQuery("get_results", "SELECT a.*, b.Nama AS Jenis, c.*, SUM(c.SisaStok) AS TotalSisaStok FROM tb_barang a, tb_jenis_material b, tb_stok_gudang c WHERE a.IDJenis=b.IDMaterial AND a.IDBarang>0 AND a.IDBarang=c.IDBarang AND c.SisaStok>0 $cond GROUP BY c.IDBarang ORDER BY b.Nama ASC, a.Nama ASC, a.IDBarang ASC");
            if ($query) {
                $i = 1;
                $grandTotal = 0;
