...
<html>
    ...
    <body>
        ...
        <div class="laporanTitle">
            <h1 class="underline">** REKAP SURAT PERINTAH PERJALANAN DINAS **</h1><?php echo $subtitle; ?>
        </div>
        <table class="tabelList2" cellpadding="0" cellspacing="0">
            ...
            <tbody>
                <?php
                $query = newQuery("get_results","SELECT *, DATE_FORMAT(TanggalMulai, '%d/%m/%Y') AS TanggalMulaiID, DATE_FORMAT(TanggalAkhir, '%d/%m/%Y') AS TanggalAkhirID FROM tb_surat_perintah_perjalanan_dinas $cond ORDER BY IDSPPD ASC");
                if($query){
                    $i=1;
