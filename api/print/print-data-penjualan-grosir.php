...
<html>
    ...
    <body>
        ...
        <div class="laporanTitle">
            <h1 class="underline">** LAPORAN DATA PENJUALAN GROSIR **</h1><?php echo $subtitle; ?>
        </div>
        <table class="tabelList2" cellpadding="0" cellspacing="0">
            <thead>
                <tr>
                    <th width="100">No.Penjualan</th>
                    <th>Pelanggan</th>
            ...
            <tbody>
                <?php
                $query = newQuery("get_results","SELECT a.*, b.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID FROM tb_penjualan a, tb_pelanggan b WHERE a.IDPelanggan=b.IDPelanggan AND a.Tipe='2' $cond ORDER BY IDPenjualan ASC");
                if($query){
                    $i=1;
                    $totalItem = 0;
                    $totalNilai = 0;
