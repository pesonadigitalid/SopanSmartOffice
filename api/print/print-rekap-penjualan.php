...
<body>
    ...
    <div class="laporanTitle">
        <h1 class="underline">** LAPORAN OUTSTANDING SPB **</h1><?php echo $subtitle; ?>
    </div>
    <table class="tabelList2" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php
            $space = "&nbsp;&nbsp;&nbsp;";
            $query = newQuery("get_results", "SELECT a.*, b.*, DATE_FORMAT(a.Tanggal,'%d/%m/%Y') AS TanggalID, a.Jenis AS JenisSPB FROM tb_penjualan a, tb_pelanggan b WHERE a.IDPelanggan=b.IDPelanggan AND a.Tipe='1' AND a.DeletedDate IS NULL $cond ORDER BY IDPenjualan ASC");
            if ($query) {
                $i = 1;
                $totalItem = 0;
                $totalTerkirim = 0;
                ...
                foreach ($query as $data) {
                    ...
                    $qDetail = newQuery("get_results", "SELECT * FROM tb_penjualan_detail WHERE NoPenjualan='" . $data->NoPenjualan . "'");
                    if ($qDetail) {
                        $i = 0;
                        foreach ($qDetail as $dDetail) {
                            ...
                            if ($dBarang->IsBarang == "1" && !$isPaket) {
                                $terkirim = newQuery("get_var", "SELECT SUM(Qty) FROM tb_penjualan_surat_jalan_detail WHERE IDBarang='" . $dDetail->IDBarang . "' AND IsChild='0' AND NoSuratJalan IN (SELECT NoSuratJalan FROM tb_penjualan_surat_jalan WHERE IDPenjualan='" . $data->IDPenjualan . "' AND DeletedDate IS NULL)");
                                if (!$terkirim) $terkirim = 0;
                                if ($terkirim > $dDetail->Qty && $dDetail->Harga == "0") $terkirim = $dDetail->Qty;
            ...
            ?>
                            <tr>
                                <?php if ($i == 1) { ?>
                                    <td style="text-align: center;"><strong><?php echo $data->NoPenjualan; ?></strong></td>
                                    <td style="text-align: center;"><strong><?php echo $data->JenisSPB; ?></strong></td>
                                    <td style="text-align: center;"><?php echo $data->NoPOKonsumen; ?></td>
                                ...
                                <td style="text-align: right;"><?php echo number_format($dDetail->Harga, 2); ?></td>
                            ...
                            if ($isPaket) {
                                foreach ($isPaket as $dataPaket) {
                                    ...
                                    $terkirim = newQuery("get_var", "SELECT SUM(Qty) FROM tb_penjualan_surat_jalan_detail WHERE IDBarang='" . $dataPaket->IDBarang . "' AND NoSuratJalan IN (SELECT NoSuratJalan FROM tb_penjualan_surat_jalan WHERE IDPenjualan='" . $data->IDPenjualan . "') AND IsChild='1' AND (IDParent='$dDetail->IDBarang' OR IDParent IS NULL OR IDParent='0')");
                                    if (!$terkirim) $terkirim = 0;
                                    $totalTerkirim += $terkirim;
                                    $outstanding =  $dDetail->Qty - $terkirim;
                                    $totalSisa += $outstanding;
                            ?>
                                    <tr>
                                        ...
                                        <td></td>
                                        <td></td>
                                        <td><?php echo $space . " * " . $dBarang->Nama; ?></td>
                                        <td style="text-align: center;"><?php echo $dDetail->Qty; ?></td>
                                        <td style="text-align: center;"><?php echo $terkirim; ?></td>
                                        <td style="text-align: center;"><?php echo $outstanding; ?></td>
                    ...
                    <tr class="highlight">
                        <td colspan='7' style="text-align: right;"><strong>T. PEMBAYARAN : </strong></td>
                        <td style="text-align: right;"><?php echo number_format($data->Pembayaran, 2); ?></td>
                        <td style="text-align: right;"><strong>SISA PIUTANG : </strong></td>
                        <td style="text-align: right;"><?php echo number_format($data->Sisa, 2); ?></td>
                    </tr>
            <?php
                }
            } else {
                echo "<tr><td colspan='10'>Tidak ada data yang dapat ditampilkan...</td></tr>";
            }
            ?>
            <!--
                <tr>
