tripApp.controller('PurchaseOrderController', function ($rootScope, $scope, $q, $routeParams, $route, $http, ngToast, CommonServices) {
  ...
  $scope.doPrint2 = function () {
    window.open($rootScope.baseURL + 'api/print/print-data-po.php?datestart=' + $scope.datestart + '&dateend=' + $scope.dateend + '&kode_proyek=' + $scope.kode_proyek + '&supplier=' + $scope.supplier + '&kategori=' + $scope.kategori, 'Print', 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=1000,height=450');
  }
});

tripApp.controller('PurchaseOrderNewController', function ($rootScope, $scope, $q, $routeParams, $rootScope, $route, $http, ngToast, CommonServices) {
  var cartArray = [];
  var noUrut = 1;
  ...
  $scope.refreshDataBarang = function () {
    $q.all({
      apiv1: $http.get('api/purchase-order/load-all-requirement.php?spb=' + $scope.spb + '&isMMSMaterialBantu=' + $scope.isMMSMaterialBantu)
  ...
  $scope.loadAll = function () {
    $q.all({
      apiv1: $http.get(`api/purchase-order/load-all-requirement.php?isMMSMaterialBantu=${$scope.isMMSMaterialBantu}`)
    }).then(function (results) {
      $scope.data_barang = results.apiv1.data.barang;
      ...
      if (cartArray.length === 0) {
        ...
        $http({
          ...
          if (data == "1") {
            ...
            window.document.location = '#/data-purchase-order/';
          } else if (data == "2") {
            $scope.processing = false;
            ngToast.create({
              className: 'danger',
...
tripApp.controller('PurchaseOrderDetailController', function ($rootScope, $scope, $route, $routeParams, $http, ngToast, $location, $timeout) {
  $('.datepick').datepicker({
    format: 'dd/mm/yyyy',
    showOtherMonths: true,
    selectOtherMonths: true,
...
tripApp.controller('PurchaseOrderEditController', function ($rootScope, $scope, $route, $routeParams, $http, ngToast, $q, CommonServices) {
  var cartArray = [];
  var noUrut = 0;
