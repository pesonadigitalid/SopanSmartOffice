tripApp.controller('ReturnPOController', function($rootScope, $scope, $q, $routeParams, $route, $http, ngToast) {
  $('.datepick').datepicker({
    format: 'dd/mm/yyyy',
  ...
  $scope.getdata = function() {
    $http.get('api/return-po/data-return-po.php?datestart=' + $scope.datestart + '&dateend=' + $scope.dateend + '&kode_proyek=' + $scope.kode_proyek + '&supplier=' + $scope.supplier + '&status=' + $scope.filterstatus).success(function(data, status) {
      $scope.data_return = data.returnPO;
  ...
  $scope.doPrint2 = function() {
    window.open($rootScope.baseURL + 'api/print/print-data-return-po.php?datestart=' + $scope.datestart + '&dateend=' + $scope.dateend + '&supplier=' + $scope.supplier, 'Print', 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=1000,height=450');
  }
});

tripApp.controller('ReturnPONewController', function($rootScope, $scope, $q, $routeParams, $rootScope, $route, $http, ngToast) {
  var cartArray = [];
  var noUrut = 1;
  ...
  $q.all({
    apiv1: $http.get('api/purchase-order/load-all-requirement.php')
...
/*
  $scope.refreshDataBarang = function() {
    if ($scope.supplier != "") {
      $http.get('api/return-po/load-all-requirement.php?supplier=' + $scope.supplier).success(function(data, status) {
      ...
      if (cartArray.length === 0) {
        ...
        $http({
          ...
          if (data.res == "1") {
            ...
            window.document.location = '#/data-return-barang/';
          } else {
            $scope.processing = false;
            ngToast.create({
              className: 'danger',
...
tripApp.controller('ReturnPODetailController', function($rootScope, $scope, $route, $routeParams, $http, ngToast, $location, $timeout) {
  $('.datepick').datepicker({
    format: 'dd/mm/yyyy',
    showOtherMonths: true,
    selectOtherMonths: true,
  ...
  $scope.processing = false;
  $http.get('api/return-po/detail.php?id=' + $routeParams.returnPOId).success(function(data, status) {
    $scope.no_return = data.detail.no_return;
    $scope.no_return_konsumen = data.detail.no_return_konsumen;
